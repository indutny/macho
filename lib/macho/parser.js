...
Parser.prototype.execute = function execute(buf) {
  ...
  hdr.cmds = this.parseCommands(hdr, hdr.body);
  delete hdr.body;
...
Parser.prototype.parseCommands = function parseCommands(hdr, buf) {
  var cmds = [];
  ...
  for (var offset = 0, i = 0; offset + 8 < buf.length, i < hdr.ncmds; i++) {
    ...
    cmds.push(this.parseCommand(type, body));
  }
...
Parser.prototype.parseCommand = function parseCommand(type, buf) {
  if (type === 'segment')
    return this.parseSegmentCmd(type, buf);
...
Parser.prototype.parseSegmentCmd = function parseSegmentCmd(type, buf) {
